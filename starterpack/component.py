...
def report():
    ...
    for comp in sorted(ALL.values(), key=lambda c: c.days_since_update):
        print(
            " {:22}{:4}   {:15}{:30}".format(
                comp.name[:19], comp.days_since_update, comp.version, comp.filename[:30]
            )
        )
...
_template = collections.namedtuple(
    "Component",
    [
        "category",
        "name",
...
def _component(data):
    ...
    config["host"] = config.get("host") or (
        "dffd"
        if isinstance(config.get("ident"), int)
        else ("github-source" if category == "graphics" else "github-asset")
    )
...
def get_globals():
    ...
    if paths.ARGS.stable and "DFHack" in all_comps:
        ...
        if target_ver != df_ver:
            if re.match(r"0\.\d\d\.\d\d", target_ver):
                ...
                all_comps["Dwarf Fortress"] = all_comps["Dwarf Fortress"]._replace(
                    version=target_ver,
                    dl_link=link,
                    filename=fname,
    ...
    if "DFHack" in all_comps and (
        all_comps["Dwarf Fortress"].version not in all_comps["DFHack"].version
    ):
    ...
    for t in ("files", "graphics", "utilities"):
        yield sorted(
            {c for c in all_comps.values() if c.category == t}, key=lambda c: c.name
        )
